...
public:
  void OnCmdPos(const msgs::Double &_msg);

  /// \brief Callback for actuator position subscription
  /// \param[in] _msg Position message
public:
  void OnActuatorPos(const msgs::Actuators &_msg);
                                        ...
                                        EventManager & /*_eventMgr*/)
{
      ...
      _sdf->Get<bool>("use_actuator_msg"))
  {
    ...
    {
      gzerr << "Please specify an actuator_number" << "to use Actuator position message control." << std::endl;
    }
  }

  // Subscribe to commands
  std::string topic;
  if ((!_sdf->HasElement("sub_topic")) && (!_sdf->HasElement("topic")) && (!this->dataPtr->useActuatorMsg))
  {
  ...
  if (_sdf->HasElement("torque_topic"))
    {
        this->topic_name = _sdf->Get<std::string>("torque_topic");
    }
    else
    {
        this->topic_name = "/joint_torque";
    }

  // Initialize the publisher
  torquePub = node.Advertise<gz::msgs::Double>(topic_name);

  if (!torquePub)
  {
    gzerr << "Failed to create publisher on topic: " << topic_name << std::endl;
  }
  else
  {
    gzdbg << "Publishing applied torques to: " << topic_name << std::endl;
  }
    ...
    EntityComponentManager &_ecm)
{
      ...
      JointPositionControllerPrivate::OperationMode::ABS)
  {
    ...
    {
      targetVel = (error < 0) ? this->dataPtr->posPid.CmdMax() : -this->dataPtr->posPid.CmdMax();
    }
    else
    {
      targetVel = -error / dt;
  ...
  for (Entity joint : this->dataPtr->jointEntities)
  {
    ...
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    // THIS IS WHERE WE ARE GOING THE FORCE TO PUBLISH TO A GAZEBO TOPIC
    gz::msgs::Double msg;
    msg.set_data(force);
